<?php
  //
  if (empty($_POST["nam"])) {
    header('location:registration.php.php?nameerr=Please enter your name');
  }
  else if (empty($_POST["mail"])) {
    header('location:registration.php.php?mailerr=Please enter valid mail&NAME='.$_POST["nam"]);
  }
  elseif (!filter_var($_POST["mail"],FILTER_VALIDATE_EMAIL)) {
    header('location:registration.php.php?mailerr=Its a invalide Email&NAME='.$_POST["nam"]);
  }
  else if (empty($_POST["pass"])) {
    header('location:registration.php.php?pass=password is empty&NAME='.$_POST["nam"].'&EMAIL='.$_POST["mail"]);
  }
  // elseif (!preg_match('/^(?=.[a-z])(?=.[A-Z])(?=.\d)(?=.[^A-Za-z\d])[\s\S]{6,16}$/',$_POST["pass"])) {
  //       header('location:registration.php.php?pass=Please give a valid pssword&NAME='.$_POST["nam"].'&EMAIL='.$_POST["mail"]);
  // }
  else if (empty($_POST["repass"])) {
    header('location:registration.php.php?repass=Type the password again&NAME='.$_POST["nam"].'&EMAIL='.$_POST["mail"].'&PASS='.$_POST["pass"]);
  }
  else if ($_POST["pass"]!=$_POST["repass"]) {
    header('location:registration.php.php?repass=Password does not matched&NAME='.$_POST["nam"].'&EMAIL='.$_POST["mail"].'&PASS='.$_POST["pass"]);
  }
  elseif (empty($_POST['mess'])) {
        header('location:registration.php.php?MESS=Enter your messge&NAME='.$_POST["nam"].'&EMAIL='.$_POST["mail"].'&PASS='.$_POST["pass"]);
  }
  elseif ($_POST['day']=='day' || $_POST['month']=='month' || $_POST['year']=='year') {
      header('location:registration.php.php?date=Select date&NAME='.$_POST["nam"].'&EMAIL='.$_POST["mail"].'&PASS='.$_POST["pass"]);
  }
  elseif (empty($_POST["gridRadios"])) {
    header('location:registration.php.php?GENDER=Please select your genderd&NAME='.$_POST["nam"].'&EMAIL='.$_POST["mail"].'&PASS='.$_POST["pass"]);
  }
  elseif (empty($_POST["check"])) {
    header('location:registration.php.php?CHECK=Please select food&NAME='.$_POST["nam"].'&EMAIL='.$_POST["mail"].'&PASS='.$_POST["pass"]);
  }
