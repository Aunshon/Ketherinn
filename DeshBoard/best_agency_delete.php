<?php
  ...
  $id=$_GET['id'];
  $select_photo="SELECT photo FROM best_agency WHERE id=$id";
  ...
  $photoDeleteLocation="best_agency_pic/".$currentPhot['photo'];
  unlink($photoDeleteLocation);


  $deleteQuery="DELETE FROM best_agency WHERE id=$id";
  $deleteResult=mysqli_query($connectionQuery,$deleteQuery);
  if (!$deleteQuery) {
    echo "Cound not delete this Data";
    header("Refresh:2; URL=best_agency.php");
  }
  else {
